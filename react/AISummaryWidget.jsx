 ...
 * <AISummaryWidget siteUrl="https://yoursite.com" siteName="Your Site" />
 */
...
export function AISummaryWidget({
  ...
  const getPrompt = () => {
    ...
    let generatedPrompt = `Review ${siteName} (${url}). Tell me about their services, what they do, and how they can help clients.`;
    
    if (founderName) {
      generatedPrompt += ` Also find the founder ${founderName}'s LinkedIn profile.`;
    }
  ...
  const platforms = [
    { name: 'ChatGPT', key: 'chatgpt', url: `https://chat.openai.com/?q=${encodedPrompt}` },
    { name: 'Claude', key: 'claude', url: `https://claude.ai/new?q=${encodedPrompt}` },
    { name: 'Perplexity', key: 'perplexity', url: `https://www.perplexity.ai/search?q=${encodedPrompt}` },
    { name: 'Gemini', key: 'gemini', url: `https://gemini.google.com/app?q=${encodedPrompt}` },
    { name: 'Grok', key: 'grok', url: `https://grok.com/?q=${encodedPrompt}` },
  ];

  const [hoveredIcon, setHoveredIcon] = React.useState(null);

  return (
    <div className={className} style={{ ...styles.widget(theme), ...style }}>
      <p style={styles.title(theme)}>
        Request an AI summary of {siteName}
      </p>
      ...
      <div style={styles.icons}>
        {platforms.map((platform) => (
          <a
            ...
            title={platform.name}
            style={{
              ...styles.iconLink(theme),
              background: hoveredIcon === platform.key 
                ? (theme === 'dark' ? 'rgba(255,255,255,0.1)' : '#f5f5f5')
                : 'transparent',
              transform: hoveredIcon === platform.key ? 'translateY(-2px)' : 'none',
            }}
            onMouseEnter={() => setHoveredIcon(platform.key)}
            onMouseLeave={() => setHoveredIcon(null)}
          >
            <span style={styles.svg}>{icons[platform.key]}</span>
          </a>
        ))}
      </div>
...
/**
 * @typedef {Object} AISummaryWidgetProps
 * @property {string} [siteUrl] - Your website URL
 * @property {string} [siteName] - Your website/company name
 * @property {string} [prompt] - Custom prompt (overrides auto-generated)
 * @property {string} [founderName] - Founder's name to include in prompt
 * @property {string} [founderLinkedIn] - Founder's LinkedIn URL
 * @property {'light' | 'dark'} [theme] - Widget theme
 * @property {boolean} [showFooter] - Show "Powered by Realmonkey" footer
